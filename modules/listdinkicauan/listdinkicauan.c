...
void copyListKicauan(ListDinKicauan lIn, ListDinKicauan *lOut) {
    CreateListDinKicauan(lOut, lIn.capacity);

    int i;
    for (i = 0; i < listLengthKicauan(lIn); i++) {
...
void insertLastKicauan(ListDinKicauan *l, ElTypeKicauan val) {
    if (!isFullKicauan(*l)) {
        ...
        ListDinKicauan lcopy;
        copyListKicauan(*l, &lcopy);

        int cap = l->capacity;
        int neff = l->nEff;

        dealocateListKicauan(l);
        CreateListDinKicauan(l, cap + 10);

        int i;
        for (i = 0; i < neff; i++) {
            l->buffer[i] = lcopy.buffer[i];
        }
        l->buffer[neff] = val;
        l->nEff = neff + 1;
    }
